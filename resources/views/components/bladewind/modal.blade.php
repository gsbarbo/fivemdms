...
if ($ok_button_action !== 'close') {
    $okAction = $ok_button_action . ($close_after_action == 'true' ? ';' . $okAction : '');
}
if ($cancel_button_action !== 'close') {
    $cancelAction = $cancel_button_action . ($close_after_action == 'true' ? ';' . $cancelAction : '');
}
@endphp

<span class="w-1/2 w-1/3 w-1/4 w-1/5 w-1/6"></span>
<div class="w-full h-full bg-black/40 fixed left-0 top-0 backdrop-blur-md z-40 flex bw-modal bw-{{ $name }}-modal hidden"
    aria-backdrop-can-close="{{ $backdrop_can_close }}">
    <div
        class="bg-white w-[94%] sm:{{ $sizes[$size] }} mx-auto my-auto rounded-lg drop-shadow-2xl bw-{{ $name }}">
        {{-- {{ $sizes[$size] }} --}}
        <div class="flex">
            @if ($type !== '')
        ...
        @if ($show_action_buttons == 'true')
            <div
                class="modal-footer @if ($center_action_buttons == 'true' || $size == 'small') text-center @else text-right @endif bg-gray-100 py-3 px-6 rounded-br-lg rounded-bl-lg">
                <x-bladewind::button type="secondary" size="small" onclick="{!! $cancelAction !!}"
                    css="cancel {{ $cancelCss }}">{{ $cancel_button_label }}</x-bladewind::button>

                <x-bladewind::button size="small" onclick="{!! $okAction !!}" css="okay ml-3 {{ $okCss }}">
